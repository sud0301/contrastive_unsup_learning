...
def parse_option():
    ...
    # optimization
    parser.add_argument('--learning-rate', type=float, default=3, help='learning rate')
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='cifar10', choices=['imagenet100', 'imagenet', 'cifar10'])
...
def get_loader(args):
    ...
    train_dataset = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=transform_ori)
    ...
    val_ids = np.arange(val_dataset_size)

    mask_val = np.zeros(val_ids.shape[0], dtype=np.bool)
    labels_val = np.array([val_dataset[i][1] for i in val_ids], dtype=np.int64)
    ...
    for i in class_index[:5]:
        mask_val[np.where(labels_val == i)[0]] = True

    labeled_indices_val = val_ids[mask_val]
    val_sampler = torch.utils.data.sampler.SubsetRandomSampler(labeled_indices_val)
